...
public class SelectorConfig_Test {
  ...
  public void translatesInputIntoLabelIdToRevisionMap() {
    ...
    assertEquals(100, config.map().get("G_G/A").intValue());
  }
  ...
  public void aGeneratorCanHaveSeveralDefaultLabels() {
    ...
    assertEquals(100, config.map().get("G_G/A").intValue());
    assertEquals(200, config.map().get("G_G/B").intValue());
  }
  ...
  public void canAccomodateMultipleGenerators() {
    ...
    assertEquals(501, config.map().get("First/L1").intValue());
    assertEquals(502, config.map().get("Second/L2").intValue());
  }
}
